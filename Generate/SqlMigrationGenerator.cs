    ...
    internal class SqlMigrationGenerator
    {
        ...
        public string Generate()
        {
            ...
            foreach (Table table in database.Tables)
            {
                ...
                {
                    AppendLine(string.Format("\t.With{0}{1}NullableColumn(\"{2}\", {3})", column.InPrimaryKey ? "PrimaryKey" : string.Empty, column.Nullable ? string.Empty : "Not", column.Name, GetDbTypeExpression(column)), ref migration);
                }
            }
            return migration;
        }
        ...
        private static string GetDbTypeExpression(Column column)
        {
            ...
            return Enum.GetName(typeof(DbType), dbType);
        }
